...
func BookingCar(c *gin.Context) {
	...
	if end.Before(start)||end.Equal(start) {
		c.JSON(http.StatusBadRequest, gin.H{"status":"error", "message": "Booking Times Unrealistic"})
		return
	}
	// Query the database using Gorm
	var results []orm.Booking
	orm.Db.Where("car_id = ? AND ((start <= ? AND end >= ?)OR(start >=? AND start <=?)OR(end >=? OR end <=?))", json.CarID, start,
		end).Find(&results)
